...
/**
 * kann mit bz2 um, konnte Archive7z nicht :-(
 */
/**
 ...
 * @package   cmanley
 *            siehe:  https://github.com/cmanley/PHP-SevenZipArchive/blob/master/SevenZipArchive.php
 */
...
class SevenZipArchive implements \Iterator
{
    ...
    protected function _list()
    {
        ...
        */
        $cmd .= ' ' . escapeshellarg($this->file);// . (($this->password) ? '-p' . $this->password : '');
    ...
    public function extractTo($destination, $names = null)
    {
        ...
        $rc = null;
        $output = array();
        $cmd = $this->binary . ' x -bd -y '. (($this->password) ? '-p' . $this->password : '') . ' -o'  . escapeshellarg($destination) . ' ' . escapeshellarg($this->file);
        if ($names)
        {
            $cmd .= ' ' . join(' ', array_map(function ($x)
                {
